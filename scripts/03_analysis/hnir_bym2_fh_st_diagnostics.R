#!/usr/bin/env Rscript
# Diagnostics (Rhat, ESS, sampler) + posterior predictive checks for hnir_bym2_fh_st fit.
#
#   Rscript scripts/03_analysis/hnir_bym2_fh_st_diagnostics.R
#
# Requires: cmdstanr, readr, dplyr, bayesplot, ggplot2
# Inputs: output/spatial_honduras_st/hnir_bym2_fh_st_fit.rds
#         output/tables/honduras_region_adolescent_logit_prevalence.csv
# Outputs: output/spatial_honduras_st/diagnostics/
...
fit_rds <- "output/spatial_honduras_st/hnir_bym2_fh_st_fit.rds"
est_csv <- "output/tables/honduras_region_adolescent_logit_prevalence.csv"
out_dir <- "output/spatial_honduras_st/diagnostics"
...
if (nrow(bad_rhat)) {
  write_csv(bad_rhat, file.path(out_dir, "warning_rhat_gt_1_01.csv"))
}

low_ess <- summ %>% filter(ess_bulk < 400 | ess_tail < 400 | is.na(ess_bulk))
if (nrow(low_ess)) {
  write_csv(low_ess, file.path(out_dir, "warning_ess_below_400.csv"))
}
...
# ----- Rebuild y, v, has_obs (same grid as fit script) -----
...
# Posterior theta at observed cells (logit scale)
...
# PPC: y_rep ~ Normal(theta, sqrt(v)) for each observed cell (Fay–Herriot replication)
y_rep <- matrix(NA_real_, nrow = S, ncol = length(idx))
for (j in seq_along(idx)) {
  y_rep[, j] <- theta_s[, j] + sqrt(v_obs[j]) * rnorm(S)
}

# Density overlay (logit direct estimates)
p1 <- ppc_dens_overlay(y_obs, y_rep) +
  labs(title = "PPC: logit direct estimates", subtitle = "y vs replicated y | theta, v") +
  theme_bw()
ggsave(file.path(out_dir, "ppc_dens_overlay_logit.png"), p1, width = 8, height = 5, dpi = 120)

# Interval plot by cell
p2 <- ppc_intervals(y_obs, y_rep, prob = 0.5, prob_outer = 0.9) +
  labs(title = "PPC: 50% & 90% intervals for replicated y (by cell)") +
  theme_bw() +
  theme(axis.text.y = element_blank())
ggsave(file.path(out_dir, "ppc_intervals_by_cell.png"), p2, width = 8, height = 6, dpi = 120)

# Scalar test quantities: mean and SD of y (across observed cells)
T_mean <- function(yp) mean(yp)
T_sd <- function(yp) sd(yp)
p3 <- ppc_stat(y_obs, y_rep, stat = "mean") + theme_bw()
p4 <- ppc_stat(y_obs, y_rep, stat = "sd") + theme_bw()
ggsave(file.path(out_dir, "ppc_stat_mean.png"), p3, width = 6, height = 4, dpi = 120)
ggsave(file.path(out_dir, "ppc_stat_sd.png"), p4, width = 6, height = 4, dpi = 120)
...
readr::write_csv(
  ...
  file.path(dirname(out_dir), "hnir_bym2_fh_st_p_total_by_wave.csv")
)
...
p_pt <- ggplot(pt_long, aes(x = survey_wave, y = prevalence)) +
  geom_violin(fill = "darkgreen", alpha = 0.35, color = NA) +
  ...
  labs(
    title = "Honduras: national total prevalence by wave (alpha + beta)",
...
p_pt2 <- ggplot(summ_pt, aes(x = survey, y = mean, ymin = q025, ymax = q975, group = 1)) +
  ...
  geom_point(size = 2) +
  labs(
    title = "Honduras: national total prevalence (mean & 95% CrI)",
...
# Standardized residuals at posterior mean theta (model check on scale)
...
message("Wrote diagnostics to ", normalizePath(out_dir, mustWork = FALSE))
