#!/usr/bin/env Rscript
# Colombia DHS 2005 IR (COIR53FL) → same analytic schema as COIR61/COIR72.
#   Rscript scripts/02_derive/derive_coir53.R
# Output: data/derived/coir53fl_derived.rds
...
input_path <- if (file.exists("data/raw/COIR53SD/COIR53FL.SAS7BDAT")) {
  "data/raw/COIR53SD/COIR53FL.SAS7BDAT"
} else {
  "data/raw/COIR53SD/coir53fl.sas7bdat"
}
output_path <- "data/derived/coir53fl_derived.rds"
if (!file.exists(input_path)) {
  stop("Missing ", input_path, " (place COIR53 IR SAS extract in data/raw/COIR53SD/).")
}
...
if (length(missing_vars) > 0) {
  stop("COIR53 missing variables: ", paste(missing_vars, collapse = ", "))
}
...
region_map <- c(
  ...
  "52" = "Narimo", "54" = "Norte de Santander", "63" = "Quindio",
  "66" = "Risaralda", "68" = "Santander", "70" = "Sucre", "73" = "Tolima",
  "76" = "Valle", "81" = "Arauca", "85" = "Casanare", "86" = "Putumayo",
  "88" = "San Andres y Providencia", "91" = "Amazonas", "94" = "Guainia",
...
coir53_derived <- data %>%
  mutate(
    ...
    pregnant = if_else(V213 == 1, 1L, if_else(V213 == 0, 0L, NA_integer_)),
    ...
    married_union = if_else(V501 %in% c(1, 2), 1L, if_else(V501 %in% c(0, 3, 4, 5), 0L, NA_integer_)),
    urban = if_else(V025 == 1, 1L, if_else(V025 == 2, 0L, NA_integer_)),
    ...
    education_c4 = if_else(V106 %in% c(0, 1, 2, 3), as.integer(V106), NA_integer_),
    agegroup_c4 = if_else(
      V012 < 20, 1L,
      if_else(
        V012 >= 20 & V012 <= 24, 2L,
    ...
    employed = if_else(V714 == 1, 1L, if_else(V714 == 0, 0L, NA_integer_)),
    parity = if_else(!is.na(V201), as.integer(V201), NA_integer_),
    using_modern_contraceptive = if_else(V313 == 3, 1L, if_else(V313 %in% c(0, 1, 2), 0L, NA_integer_)),
    unfecund = if_else(
...
saveRDS(coir53_derived, output_path)
message("Wrote ", output_path, " (n = ", nrow(coir53_derived), ")")
